...
$page_title   = 'ตั้งค่ามิเตอร์น้ำเริ่มต้น';
$current_page = 'meter_init';

// ====================================================================
// AJAX POST: บันทึก water_meter_init
// ====================================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['ajax'])) {
    header('Content-Type: application/json');
    $room_id    = (int)($_POST['room_id']    ?? 0);
    $water_init = trim($_POST['water_init'] ?? '');

    if ($room_id > 0 && is_numeric($water_init) && (float)$water_init >= 0) {
        try {
            $pdo->prepare("UPDATE rooms SET water_meter_init = ? WHERE id = ?")
                ->execute([(float)$water_init, $room_id]);
            echo json_encode(['ok' => true]);
        } catch (PDOException $e) {
            error_log('meter_init: ' . $e->getMessage());
...
$params = ['fdorm_c' => $filter_dorm, 'fdorm_v' => $filter_dorm,
           'ffloor_c' => $filter_floor, 'ffloor_v' => $filter_floor];

$stmt = $pdo->prepare("
    SELECT r.id AS room_id, r.room_number, r.water_meter_init, r.floor,
           d.name AS dorm_name
    FROM rooms r
...
$hasInit      = count(array_filter($rooms, fn($r) => $r['water_meter_init'] !== null));
...
<div class="page-header">
    <div>
        <h2><i class="bi bi-123 me-2" style="color:#0d9488;"></i>ตั้งค่ามิเตอร์น้ำเริ่มต้น</h2>
        <p class="page-desc">กรอกเลขมิเตอร์ก่อนเริ่มใช้ระบบ — ใช้สำหรับคำนวณหน่วยน้ำเดือนแรก</p>
    </div>
</div>
...
<div class="stat-strip">
    <div class="stat-chip">
        <div class="sc-icon" style="background:#f0fdfa;color:#0d9488;"><i class="bi bi-house-fill"></i></div>
...
<!-- Filter bar -->
<form method="GET" action="meter_init.php" class="filter-bar">
    ...
    <a href="meter_init.php" class="btn-filter-clear"><i class="bi bi-x-circle"></i> ล้างตัวกรอง</a>
    <?php foreach ($allDorms as $d): ?>
...
<div class="panel">
    ...
    <div class="table-responsive">
        <table class="init-table">
            ...
            <?php endforeach; ?>
            <?php if (empty($rooms)): ?>
                <tr><td colspan="4" style="text-align:center;padding:40px;color:#94a3b8;">ไม่พบข้อมูลห้องพัก</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
async function saveInit(form, event) {
    ...
    try {
        const res  = await fetch('meter_init.php', { method: 'POST', body: new FormData(form) });
        ...
        if (data.ok) {
            ...
            setTimeout(() => {
                ...
                // focus next row
                const nextInput = form.closest('tr').nextElementSibling?.querySelector('input[name="water_init"]');
